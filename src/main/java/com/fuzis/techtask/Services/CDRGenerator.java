...
/**
 * Сервис для генерации CDR записей и абонентов в БД
 */
@Service
public class CDRGenerator {
    ...
    /**
     * Данный метод генерирует строку длиной до 9 символов, состоящую из указанного количество случайных цифр
     *
     * @param length требуемая длина строки
     * @return сгенерированную строку или {@code IllegalArgumentException}, если {@code length} > 9
     */
    ...
    /**
     * Данный метод генерирует указанное количество абонентов (Entity Client) и сохраняет их в базу данных
     *
     * @param cnt требуемое количество сгенерированных пользователей
     */
    ...
    /**
     * Данный метод генерирует указанное количество CDR записей (Entity CDRRecord) и сохраняет их в базу данных
     *
     * @param cnt требуемое количество сгенерированных CDR записей
     */
    public void generateCDRs(int cnt, LocalDateTime startDate, LocalDateTime finalDate, long maxTalkTimeSeconds) {
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            CDRRecord cdr = new CDRRecord(rand.nextBoolean() ? CDRRecord.CallType.Income : CDRRecord.CallType.Outcome, clients.get(rand.nextInt(clients.size())).getPhoneNumber(), (rand.nextBoolean() ? "+" : "") + nextNumberString(4) + nextNumberString(7), callStartDate, callEndDate);
